...
The hottest new tools in the software world for 2022-2023 are Large Language Models, and [ChatGPT](https://beta.openai.com) in particular.
...
- **Command**: Detailing the action to be taken by ChatGPT. While the previous 4 items are permanent, this one changes for each interaction with the API, you can write text but you could also write the current state of your system as defined by the Application and the Data Model, ChatGPT can consume JSON too.
...
Later I found the [OpenAI Playground](https://beta.openai.com/playground?lang=json), which is better for debugging and tweaking the configuration; lowering the `temperature` field makes the responses more deterministic, and the engine seems faster that way too. Use the playground, or a better tool than `curl` such as [httpie](https://httpie.io/app).
...
I later wrapped this in an Apple Shortcut to use while cooking. I uploaded it to [this blog](/assets/files/talk.shortcut); it just needs an API key.
...
![watercolor horse](/assets/images/horse.webp)
