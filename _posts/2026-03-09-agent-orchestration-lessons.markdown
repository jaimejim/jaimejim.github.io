...
A base skill with writing conventions is always active. Specialized skills layer on top depending on the task. Activation is trigger-based: a hashtag in the prompt tells the agent which skills to load. The user controls what the agent focuses on.

Each skill is short, 30-80 lines. Because only a few activate per session, the token overhead stays low compared to loading everything at once. Simple pattern, but the savings add up across a full day of sessions.
